...
/******************************************************************************/
	...
	static private Stream<IDataContainer> ValuesIterator(IDataContainer input){
		...
		if (nbt instanceof CollectionTag<?> list)
			...
			return compound.getAllKeys().stream().map(compound::get).map(RawDataContainer::OfNullable);
		else
			return null;
	}
	...
	static private Stream<IDataContainer> KeysIterator(IDataContainer input){
		...
		if (nbt instanceof CollectionTag<?> list)
			...
			return compound.getAllKeys().stream().map(RawDataContainer::OfNullable);
		else
			return null;
	}
	...
	static private Stream<IDataContainer> EntriesIterator(IDataContainer input){
		...
		if (nbt instanceof CollectionTag<?> list)
			...
			return compound.getAllKeys().stream().map(key->CompoundEntry(key, compound)).map(RawDataContainer::OfNullable);
		else
			return null;
	}
